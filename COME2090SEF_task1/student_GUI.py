...
class StudentGUI:
    ...
    def action_record(self):
        ...
        if not records:
            ...
            for cid, r in records.items():
                ...
                display_str += f"- [{cid}] {course_name} | Status: {status}\n"
    ...
    def action_rate(self):
        ...
        if c_id:
            rate, is_failed = self.system.get_student_attendance_rate(self.current_user.user_id, c_id)
            
            status_msg = "FAILED (Attendance < 80%)" if is_failed else "PASS (Attendance >= 80%)"
            
            display_str = f"[Attendance Analysis for {c_id}]\n\n"
            display_str += f"Current Rate: {rate:.1f}%\n"
            display_str += f"Course Status: {status_msg}\n"
            self.update_display(display_str)
