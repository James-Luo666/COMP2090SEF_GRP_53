...
class TeacherGUI:
    ...
    def action_attendance(self):
        c_id = simpledialog.askstring("Input", "Enter Course ID:")
        s_id = simpledialog.askstring("Input", "Enter Student ID:")
        status = simpledialog.askstring("Input", "Enter Status (Present/Absent/Leave):")
        
        if c_id and s_id and status:
            success, msg = self.system.edit_attendance(self.current_user.user_id, c_id, s_id, status)
            messagebox.showinfo("Result", msg)

    def action_statistics(self):
        c_id = simpledialog.askstring("Input", "Enter Course ID for statistics:")
        if c_id:
    ...
    def action_approve_leave(self):
        ...
        s_id = simpledialog.askstring("Input", "Enter Student ID:")
        if c_id and s_id:
            is_approved = messagebox.askyesno("Approval", f"Approve leave for {s_id} in {c_id}?")
            success, msg = self.system.approve_leave_application(self.current_user.user_id, c_id, s_id, is_approved)
            messagebox.showinfo("Result", msg)

    def action_export(self):
        c_id = simpledialog.askstring("Input", "Enter Course ID to export:")
        if c_id:
